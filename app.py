...
# Function to find all unique triangles (3-currency cycles)
...
def calculate_profit(ex, tickers, ob_cache, cycle, fee, min_volume, trade_size):
    ...
    for i in range(3):
        ...
        if pair not in ob_cache:
            ob_cache[pair] = ex.fetch_order_book(pair, limit=1)
...
with col1:
    exchange_name = st.selectbox("Select Exchange", ["binance", "kraken", "kucoin", "bitfinex", "huobi", "gateio"])
with col2:
...
if st.button("Scan for Opportunities"):
    with st.spinner("Scanning exchanges for triangular opportunities..."):
        try:
            ...
            ex.load_markets()
            triangles = find_triangles(ex.markets)
