# app.py — Triangular Arbitrage Scanner (Strict Spot Validation + Active Pair Check + Kraken)
...
def enumerate_triangles(ex) -> list:
    """Find strictly valid triangles (A,B,C) with active spot pairs."""
    currencies = set()
    graph = defaultdict(set)

    for symbol, m in ex.markets.items():
        if not m.get("spot", False):  # only spot
            continue
        if not m.get("active", True):  # skip inactive
            continue
        if "/" not in symbol:
            continue
        ...
        for b in graph[a]:
            for c in graph[b]:
                ...
                if a in graph[c]:  # completes the cycle
                    ...
                    if key not in seen:
                        # validate each leg is active and spot
                        s1 = get_pair_for(ex, a, b)
                        s2 = get_pair_for(ex, b, c)
                        s3 = get_pair_for(ex, c, a)
                        if (
                            s1 and ex.markets[s1].get("spot", False) and ex.markets[s1].get("active", True)
                            and s2 and ex.markets[s2].get("spot", False) and ex.markets[s2].get("active", True)
                            and s3 and ex.markets[s3].get("spot", False) and ex.markets[s3].get("active", True)
                        ):
                            seen.add(key)
...
if scan_btn:
    ...
    if df is None or df.empty:
        ...
        # Highlight positive/negative profits
        def highlight_profit(val):
            color = "green" if val > 0 else "red"
            return f"color: {color}; font-weight: bold;"

        styled = df.style.applymap(highlight_profit, subset=["Profit % AFTER Fees"])
